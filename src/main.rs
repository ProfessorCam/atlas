...
fn main() -> eframe::Result<()> {
    ...
    // CLI: `atlas [FLAGS] [PATH]`
    //   PATH             scan it immediately on startup
    //   --no-free-space  start with the free-space block hidden
    let args: Vec<String> = std::env::args().skip(1).collect();
    let initial_path = args.iter().find(|a| !a.starts_with('-')).cloned();
    let free_space_override = if args.iter().any(|a| a == "--no-free-space") {
        Some(false)
    } else {
        None
    };
    ...
    eframe::run_native(
        "Atlas",
        options,
        Box::new(move |cc| Ok(Box::new(AtlasApp::new(cc, initial_path, free_space_override)))),
    )
}
